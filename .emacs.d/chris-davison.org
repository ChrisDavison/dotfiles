...
(setq org-capture-templates
      `(
        ;; Put the capture groups/clusters here, rather than above the specific bindings
        ;; so that I can easily see the high level mappings that I'm currently using.
        ("t" "Todos")
        ("c" "Calendar")
        ("w" "Work")
        ("g" "Games")

        ("tt" "Task" item (file+headline "inbox.org" "Tasks")
         "[ ] %^{TASK}" :immediate-finish t :unnarrowed t)
        ("td" "Task (Dated)" item (file+headline "inbox.org" "Tasks")
         "[ ] %^t %^{TASK}" :immediate-finish t  :unnarrowed t)
        ("tT" "Project" entry (file "inbox.org")
         "* TODO %^{TASK}\n%?" :empty-lines 1)
        ("ta" "Add to Anki" item (file+headline "inbox.org" "Tasks")
         "[ ] *(Anki)* %^{knowledge}" :immediate-finish t :unnarrowed t)
        ("tb" "Buy" item (file+headline "inbox.org" "Tasks")
         "[ ] *($)* %^{Buy}" :immediate-finish t :unnarrowed t)
        ("tc" "Custom task (free writing)" item (file+headline "inbox.org" "Tasks")
         "[ ] %?" :unnarrowed t)
        ("tu" "URL" item (file+headline "inbox.org" "Tasks")
         "[ ] [[%c][%^{DESCRIPTION}]]" :unnarrowed t)

        ("ww" "WORK Task" item (file+headline "work.org" "TASKS")
         "[ ] %^{Task}" :immediate-finish t :unnarrowed t)
        ("wW" "WORK Project" entry (file "work.org")
         "* TODO %^{Task}\n%?" :empty-lines 1)
        ("wt" "Thesis task" item (file+olp "thesis.org" "UNFILED")
         "[ ] %^{TASK}" :immediate-finish t :unnarrowed t)

        ;; CALENDAR STUFF;;

        ("cc" "Calendar Event" entry (file+headline "calendar.org" "Calendar")
         "* TODO %^t %^{EVENT}")
        ("ca" "Archive (Noteable work done)" item (file+headline "calendar.org" "Work Done")
         "%u %^{TASK}" :immediate-finish t)

        ("n" "Note" plain (file+headline "inbox.org" "Notes")
         "%?" :empty-lines 1)

        ;; Datetree of YYYY / YYYY-MM MONTHNAME / YYYY-MM-DD DAYNAME
        ("l" "Logbook" plain (file+headline "logbook.org" ,(format-time-string "%Y-%m-%d %A"))
...
(defhydra cd/hydra/main (:exit t)
  ...
  ("t" (progn (org-capture nil "tt")) "InboxTask")
...
** Mix monospace and proportional fonts (code vs prose)
#+BEGIN_SRC emacs-lisp
