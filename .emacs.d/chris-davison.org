...
** Startup
Set up load path (with package... stuff) so we can override it.  Then, install use package and diminish (so use-package can use it from then on)
...
#+END_SRC
** Text filling (paragraph reflowing) and indenting
...
Aggressive indentation (force code to the next tabstop)
#+BEGIN_SRC emacs-lisp
...
#+end_src
* Navigation
** General Navigation config (Ivy / counsel)
...
** My utility binds
#+BEGIN_SRC emacs-lisp
(defhydra cd/zoom (:exit nil :hint t)
...
(defhydra cd/links (:exit t :hint t)
  ("c" insert-link-with-description-prompt "From clipboard")
  ("l" org-insert-link "Prompted")
  ("s" org-store-link "Store"))

(defhydra cd/windowmove (:exit t :hint t)
  ("R" (progn (split-window-right) (windmove-right)) "Split right")
  ("D" (progn (split-window-below) (windmove-down)) "Split down")
  ("n" cd/move-to-next-narrow "Next narrowed subtree" :exit nil)
  ("p" cd/move-to-previous-narrow "Previous narrowed subtree" :exit nil))
...
(defhydra  hydra-cd-util (:hint nil :exit t)
  ...
  ("ft" yafolding-toggle-element "Fold Toggle")
  ("fa" yafolding-toggle-all "Fold Toggle All")
  ("z" cd/zoom/body "Font size" :column "Appearance")
  ("C-z" (modi/toggle-one-window nil) "Zen mode" :column "Appearance")
  ("w" cd/windowmove/body "Windows" :column "Appearance")

  ("s" counsel-grep-or-swiper "GrepSwipe" :column "Search & Replace")
  ...
  (">" indent-rigidly-right-to-tab-stop "Indent to tabstop" :column "Utils" :exit nil)
  ("<" indent-rigidly-left-to-tab-stop "Dedent to tabstop" :column "Utils" :exit nil)
)

(when (package-installed-p 'alchemist)
  (defhydra cd/elixir (:exit t :hint nil)
    ("r" (if (region-active-p) 
             'alchemist-iex-send-region 'alchemist-iex-send-last-sexp) 
     "Send region")
    ("R" alchemist-iex-send-region-and-go "Send region and Go")
    ("s" alchemist-iex-run "Start repl"))
  (define-key elixir-mode-map (kbd "M-e") 'cd/elixir/body))
