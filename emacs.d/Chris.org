...
** tip
It is recommended not to just blindly copy paste.  Instead, move a section of interest into a seperate buffer and evaluate to see if you like the changes, without actually merging it into your config and potentially screwing things up.

Useful: [[https://www.gnu.org/software/emacs/manual/html_mono/eintr.html][An Introduction to Programming in Emacs Lisp]]

Also: =(require-package 'something...)= is a macro that I acquired from somewhere.  Can't remember where.
...
* ido
Disabling for now, as I think Helm could be a better all-round replacement, although IDO may be a good fallback for when Helm is not available.
#+BEGIN_SRC emacs-lisp :tangle no
  (require 'ido)
  (ido-mode t)
  (ido-everywhere t)
