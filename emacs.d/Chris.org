...
#+BEGIN_SRC emacs-lisp
(when (member "Osaka" (font-family-list))
  (set-face-attribute 'default nil :font "Osaka" :height 150))
#+END_SRC
...
Add a couple other things into autocomplete
#+BEGIN_SRC emacs-lisp
...
Useful settings, and the hooks to set it up
#+BEGIN_SRC emacs-lisp
...
** Editing Source Code
   Need to try this...from SC "I don't want to get distracted by the same code in the other window, so I want org src to use the current window."
#+begin_src emacs-lisp
...
#+BEGIN_SRC emacs-lisp :results silent
  ...
  (global-set-key (kbd "C-c w") 'helm-descbinds)
#+END_SRC
* Config: Git / Magit
