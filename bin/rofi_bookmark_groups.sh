...
if [ -z "$@" ]; then
    ...
    if [ ! -z "$option" ]; then
        case $(echo "$option" | cut -d' ' -f1) in
            ...
            Weather) firefox --new-tab "https://darksky.net/forecast/55.8259,-4.2265/uk212/en" --new-tab "https://www.metoffice.gov.uk/weather/forecast/gcuvz3bch#?nearestTo=Glasgow%20(Glasgow%20City)" --new-tab "https://www.bbc.co.uk/weather/2648579" ;;
